inherited frmCadPrazos: TfrmCadPrazos
  Caption = 'Cadastro de Prazos'
  ClientHeight = 428
  ClientWidth = 501
  ExplicitWidth = 507
  ExplicitHeight = 457
  PixelsPerInch = 96
  TextHeight = 13
  object lbCodigo: TLabel [0]
    Left = 8
    Top = 58
    Width = 33
    Height = 13
    Caption = 'Codigo'
    FocusControl = dbCodigo
  end
  object lbDescricao: TLabel [1]
    Left = 83
    Top = 58
    Width = 48
    Height = 13
    Caption = 'Descricao'
    FocusControl = dbDescricao
  end
  object lbParcelas: TLabel [2]
    Left = 8
    Top = 104
    Width = 41
    Height = 13
    Caption = 'Parcelas'
    FocusControl = dbParcelas
  end
  object lbJuros: TLabel [3]
    Left = 168
    Top = 104
    Width = 25
    Height = 13
    Caption = 'Juros'
    FocusControl = dbJuros
  end
  object Label1: TLabel [4]
    Left = 217
    Top = 104
    Width = 119
    Height = 13
    Caption = 'Indica'#231#227'o de Pagamento'
  end
  object Label2: TLabel [5]
    Left = 83
    Top = 104
    Width = 66
    Height = 13
    Caption = 'Per'#237'odo (dias)'
    FocusControl = dbJuros
  end
  object Label3: TLabel [6]
    Left = 8
    Top = 150
    Width = 105
    Height = 13
    Caption = 'Formas de pagamento'
  end
  object sbNew: TSpeedButton [7]
    Left = 168
    Top = 185
    Width = 25
    Height = 25
    Hint = 'Insere nova forma de pagamento'
    Flat = True
    Glyph.Data = {
      
      A471FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    OnClick = sbNewClick
  end
  object sbCancel: TSpeedButton [8]
    Left = 168
    Top = 305
    Width = 25
    Height = 25
    Hint = 'Cancela as altera'#231#245'es no registro'
    Flat = True
    Glyph.Data = {
      
      00FFFF00FFFF00FF1C1CA01C1CA01C1CA0FF00FFFF00FFFF00FF}
    OnClick = actCancelarExecute
  end
  object sbDelete: TSpeedButton [9]
    Left = 168
    Top = 225
    Width = 25
    Height = 25
    Hint = 'Excluir forma de pagamento'
    Flat = True
    Glyph.Data = {
      
      00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
    OnClick = actCancelarExecute
  end
  object sbGravar: TSpeedButton [10]
    Left = 168
    Top = 264
    Width = 25
    Height = 25
    Hint = 'Grava a nova forma de pagamento'
    Flat = True
    Glyph.Data = {
      
      EFE3FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    OnClick = sbGravarClick
  end
  object dbDescricao: TDBEdit [11]
    Left = 83
    Top = 77
    Width = 291
    Height = 21
    Hint = 'Descricao'
    DataField = 'PRZ_DESCRICAO'
    DataSource = dsCadastro
    ParentShowHint = False
    ShowHint = True
    TabOrder = 1
  end
  object dbParcelas: TDBEdit [12]
    Left = 8
    Top = 123
    Width = 69
    Height = 21
    Hint = 'Parcelas'
    DataField = 'PRZ_PARCELAS'
    DataSource = dsCadastro
    ParentShowHint = False
    ShowHint = True
    TabOrder = 2
  end
  object dbJuros: TDBEdit [13]
    Left = 168
    Top = 123
    Width = 43
    Height = 21
    Hint = 'Juros'
    DataField = 'PRZ_JUROS'
    DataSource = dsCadastro
    ParentShowHint = False
    ShowHint = True
    TabOrder = 4
  end
  object dbCodigo: TDBEdit [14]
    Left = 8
    Top = 77
    Width = 69
    Height = 21
    Hint = 'Codigo'
    TabStop = False
    Color = clBtnFace
    DataField = 'PRZ_CODIGO'
    DataSource = dsCadastro
    ParentShowHint = False
    ReadOnly = True
    ShowHint = True
    TabOrder = 0
  end
  inherited pnBotoes: TPanel
    Left = 380
    Height = 357
    TabOrder = 6
    ExplicitLeft = 378
    ExplicitHeight = 357
  end
  inherited btFechar: TBitBtn
    Left = 388
    Top = 378
    TabOrder = 8
    ExplicitLeft = 386
    ExplicitTop = 378
  end
  inherited ToolBar1: TToolBar
    Width = 501
    TabOrder = 7
    ExplicitWidth = 499
  end
  inherited panCaption: TPanel
    Width = 501
    TabOrder = 9
    ExplicitWidth = 499
  end
  inherited sbPrincipal: TStatusBar
    Top = 409
    Width = 501
    ExplicitTop = 409
    ExplicitWidth = 499
  end
  object cbIndicacao: TwwDBComboBox [20]
    Left = 217
    Top = 123
    Width = 157
    Height = 21
    Hint = 'indPag (NFe)'
    ShowButton = True
    Style = csDropDown
    MapList = True
    AllowClearKey = False
    ShowMatchText = True
    DataField = 'prz_indpag'
    DataSource = dsCadastro
    DropDownCount = 8
    ItemHeight = 0
    Items.Strings = (
      'Pagamento '#224' vista'#9'0'
      'Pagamento '#224' prazo'#9'1'
      'Outros'#9'2')
    Sorted = False
    TabOrder = 5
    UnboundDataType = wwDefault
  end
  object dbPeriodo: TDBEdit [21]
    Left = 83
    Top = 123
    Width = 79
    Height = 21
    Hint = 'Parcelas'
    DataField = 'prz_periodo'
    DataSource = dsCadastro
    ParentShowHint = False
    ShowHint = True
    TabOrder = 3
  end
  object GridFomas: TDBGrid [22]
    Left = 8
    Top = 169
    Width = 154
    Height = 234
    DataSource = dsFormas
    TabOrder = 11
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'frp_descricao'
        Visible = True
      end>
  end
  inherited ActionList: TActionList
    Left = 392
    inherited actAtualizar: TAction
      OnExecute = actAtualizarExecute
    end
    inherited actProcurar: TAction
      OnExecute = actProcurarExecute
    end
  end
  inherited Imagens: TImageList
    Left = 440
  end
  inherited dsCadastro: TDataSource
    DataSet = dmFinanceiro.qyPrazos
  end
  object dsFormas: TDataSource
    DataSet = dmFinanceiro.qyPrazosfrp
    OnStateChange = dsFormasStateChange
    Left = 320
  end
end
